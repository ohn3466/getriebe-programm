...
class ReportTests(unittest.TestCase):
    def test_calculation_report_contains_formulas_values_and_checks(self):
        ...
        self.assertIn("`i_soll = n_1 / n_2,soll` (21.9)", report)
        self.assertIn("`d_1 = m_n * z_1 / cos(beta)` (21.1)", report)
        self.assertIn("`epsilon_alpha = g_alpha / p_e` (21.26)", report)
    ...
    def test_live_latex_blocks_are_generated(self):
        ...
        self.assertIn(r"\text{(21.9)}", rendered)
        self.assertIn(r"\text{(21.1)}", rendered)
        self.assertIn(r"\text{(21.26)}", rendered)
